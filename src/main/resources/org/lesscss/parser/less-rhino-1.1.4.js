    ...
    return parser = {
        ...
        parse: function (str, callback) {
            ...
            if (this.imports.queue.length > 0) {
                finish = function () { callback(error, root, env) };
            } else {
                callback(error, root, env);
            }
        },
...
if (typeof(window) !== 'undefined' /* browser */ || typeof(exports) === 'undefined' /* rhino */) {
    ...
    less.Parser.importer = function (path, paths, callback, env) {
        ...
        loadStyleSheet(rootEnv,env,{ href: path, title: path, type: env.mime }, callback, true);
    };
}
...
function readFile(f){
	return String(rhinoUtil.readFile(f));
}

function loadStyleSheet(rootEnv, env, sheet, callback, reload, remaining) {
	var name=env.filename;
	name=name.replace(/\\/g,'/');
    var sheetName = name.slice(0, name.lastIndexOf('/') + 1) + sheet.href;
    var input = readFile(sheetName);
    if(!rootEnv.resources){
    	rootEnv.resources=new Array();
    } 
    rootEnv.resources.push(sheetName);
    var parser = new less.Parser({filename:sheetName,rootEnv:rootEnv});
    parser.parse(input, function (e, root, env) {
        if (e) {
            print("Error: " + e);
            return;
        }
